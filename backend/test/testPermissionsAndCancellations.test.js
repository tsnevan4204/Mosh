...
describe("EventManager - Permissions & Cancellation Flow", function () {
  ...
  before(async () => {
    ...
    console.log("\n🔍 ETH Balances:");
    console.log(`BuyerOne   (${buyerOne.address}): ${ethers.formatEther(await provider.getBalance(buyerOne.address))} ETH`);
    console.log(`BuyerTwo   (${buyerTwo.address}): ${ethers.formatEther(await provider.getBalance(buyerTwo.address))} ETH`);
    console.log(`Organizer  (${organizer.address}): ${ethers.formatEther(await provider.getBalance(organizer.address))} ETH`);
    console.log(`Owner      (${owner.address}): ${ethers.formatEther(await provider.getBalance(owner.address))} ETH`);
  ...
  it("Organizer creates a valid event with 3 tickets", async () => {
    ...
    const tx = await eventManager.connect(organizer).createEvent(metadataURI, price, 3, futureTimestamp);
  ...
  it("Buyers cannot edit or cancel the event", async () => {
    await expect(
      eventManager.connect(buyerOne).updateEventMetadataURI(eventId, "ipfs://unauthorized")
  ...
  it("Organizer can edit and cancel the event", async function () {
    ...
    const updateMetadataTx = await eventManager.connect(organizer).updateEventMetadataURI(eventId, newURI);
    ...
    const updatePriceTx = await eventManager.connect(organizer).updateTicketPrice(eventId, newPrice);
    ...
    const evt = await eventManager.events(eventId);
    console.log(`📜 Event details: Metadata URI: ${evt.metadataURI}, Ticket Price: ${ethers.formatEther(evt.ticketPrice)} ETH`);
    ...
    const cancelEventTx = await eventManager.connect(organizer).cancelEvent(eventId);
