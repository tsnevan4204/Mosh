...
describe("EventManager & Ticket (Event Lifecycle)", function () {
  ...
  it("BuyerTwo buys 3 tickets", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        console.log(`❌ Failed to buy ticket #${bought + 1}: ${err.reason || err.message}`);
        break;
      }
    }
  ...
  it("Organizer cancels the event and refunds are issued", async () => {
    ...
    console.log(`🧾 Ticket IDs: ${ticketIds.map(t => t.toString()).join(", ")}`);

    const expectedRefund = await eventManager.totalReceived(eventId);
    console.log(`💸 Total expected refund to send with cancel: ${ethers.formatEther(expectedRefund)} ETH`);
    ...
    try {
      ...
      const reason =
        err?.error?.reason ||
        err?.reason ||
        (err?.message?.includes("custom error") ? "Custom Error" : "Unknown revert");
    ...
    console.log(`💰 BuyerOne refund delta: ${ethers.formatEther(refundOne)} ETH`);
    console.log(`💰 BuyerTwo refund delta: ${ethers.formatEther(refundTwo)} ETH`);
